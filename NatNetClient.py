...
class NatNetClient:
    ...
    def __init__(self, server="169.168.37.1", multicast="239.255.42.99", commandPort=1510, dataPort=1511,
                 rigidBodyListener=None, newFrameListener=None, rigidBodyListListener=None, verbose=False, X=None):
    ...
    def __unpackRigidBody(self, data, X):
        ...
        if self.rigidBodyListener is not None:
            self.rigidBodyListener(id, pos, rot, X)
    ...
    def __unpackMocapData(self, data, X):
        ...
        for i in range(0, rigidBodyCount):
            offset += self.__unpackRigidBody(data[offset:], X)
    ...
    def __dataThreadFunction(self, sock, X):
        ...
        while self.running:
            ...
            try:
                ...
                if (len(data) >= 4):
                    self.__processMessage(data, X)

            except socket.timeout:

                pass

    def __processMessage(self, data, X):
        ...
        if (messageID == self.NAT_FRAMEOFDATA):
            ...
            self.__unpackMocapData(data[offset:], X)
    ...
    def run(self):
        ...
        X = self.X
        dataThread = Thread(target=self.__dataThreadFunction, args=(self.dataSocket, X))

        dataThread.start()

        # Create a separate thread for receiving command packets

        commandThread = Thread(target=self.__dataThreadFunction, args=(self.commandSocket, X))
